...
public class CustomCrashHandler implements Thread.UncaughtExceptionHandler {
	...
	public void uncaughtException(Thread thread, Throwable ex) {
		...
		//提示用户程序即将退出
		showToast(mContext, "很抱歉，\""+mContext.getApplicationInfo().loadLabel(mContext.getPackageManager())+"\"遭遇异常，即将退出！");
		try {
			thread.sleep(2000);
...
//		ActivityStack.popAll();
	...
	private String savaInfoToSD(Context context, Throwable ex){
		...
		if(Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)){
			File dir = new File(SDCARD_ROOT + File.separator + "crash" + File.separator + context.getPackageName() + File.separator);
			if(!dir.exists()){
				dir.mkdirs();
			}
